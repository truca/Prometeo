...
void PortListener::sendPoints(int mode, int type, int count){
    pointType=type;
    QByteArray buff(2041,0);
    mavlink_message_t msg;
    mavlink_msg_mission_count_pack(sysID, compID, &msg, uavsysID, MAV_COMP_ID_MISSIONPLANNER, type, count);
...
void PortListener::sendPoint(float lat, float lng){
    if(pointType==1)
...
void PortListener::SendWPCount(){
    QByteArray buff(2041,0);
    mavlink_message_t msg;
    mavlink_msg_mission_count_pack(sysID, compID, &msg, uavsysID, MAV_COMP_ID_MISSIONPLANNER, 1, 3);
    int len = mavlink_msg_to_send_buffer((uint8_t *)buff.data(), &msg);
    uint32_t bytes = port->write(buff.data(), len);
    if(bytes==-1)
        qWarning("error sending");
