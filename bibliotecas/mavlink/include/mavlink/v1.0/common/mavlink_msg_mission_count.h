...
{
 uint8_t type; ///< type of the point (0: WayPoint,1: ViewPoint,2: HomePoint)
...
#define MAVLINK_MESSAGE_INFO_MISSION_COUNT { \
	"MISSION_COUNT", \
	3, \
    {  {"type", NULL, MAVLINK_TYPE_UINT8_T, 0, 0, offsetof(mavlink_mission_count_t, type)}\
         { "count", NULL, MAVLINK_TYPE_UINT16_T, 0, 1, offsetof(mavlink_mission_count_t, count) }, \
         { "target_system", NULL, MAVLINK_TYPE_UINT8_T, 0, 3, offsetof(mavlink_mission_count_t, target_system) }, \
         { "target_component", NULL, MAVLINK_TYPE_UINT8_T, 0, 4, offsetof(mavlink_mission_count_t, target_component) }, \
         } \
}
...
static inline uint16_t mavlink_msg_mission_count_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
                               uint8_t target_system, uint8_t target_component, uint8_t type, uint16_t count)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[5];
    _mav_put_uint8_t(buf,0,type);
    _mav_put_uint16_t(buf, 1, count);
    ...
    return mavlink_finalize_message(msg, system_id, component_id, 5, 221);
}
                                   ...
                                   uint8_t target_system,uint8_t target_component, uint8_t type, uint16_t count)
{
	...
	msg->msgid = MAVLINK_MSG_ID_MISSION_COUNT;
    return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 5, 221);
}
...
{
    return mavlink_msg_mission_count_pack(system_id, component_id, msg, mission_count->target_system, mission_count->target_component, mission_count->type, mission_count->count);
}
...
static inline void mavlink_msg_mission_count_send(mavlink_channel_t chan, uint8_t target_system, uint8_t target_component, uint8_t type, uint16_t count)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[5];
    _mav_put_uint8_t(buf, 0, type);
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_MISSION_COUNT, buf, 5, 221);
#else
	mavlink_mission_count_t packet;
    packet.type = type;
	packet.count = count;
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_MISSION_COUNT, (const char *)&packet, 5, 221);
#endif
}
...
static inline uint16_t mavlink_msg_mission_count_get_type(const mavlink_message_t* msg)
{
...
static inline void mavlink_msg_mission_count_decode(const mavlink_message_t* msg, mavlink_mission_count_t* mission_count)
{
    ...
    mission_count->type = mavlink_msg_mission_count_get_type(msg);
...
#else
    memcpy(mission_count, _MAV_PAYLOAD(msg), 5);
#endif
}
